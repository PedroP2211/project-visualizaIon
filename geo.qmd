...
homicidios = pd.read_csv("homicidios_fixed.csv")

homicidios["datetime"] = pd.to_datetime(
    homicidios["fecha_infraccion"].astype(str) + " " + homicidios["hora_infraccion"].astype(str),
    errors="coerce"
)

print(homicidios["datetime"].isna().sum())

homicidios = homicidios.dropna(subset=["datetime"])

homicidios.to_csv("homicidios_temporal.csv", index=False)
